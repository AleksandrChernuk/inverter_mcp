...
### Фаза 2 — `vent_select_ipart_member` (рычаг для способа 1: iPart/iAssembly) — РЕАЛИЗОВАНО
Выбрать строку iPart/iAssembly по имени члена (`member`), по значениям ключевых колонок (`keys`) или
по 1-based индексу строки (`row`) на активном factory-документе; на фабрике `CreateMember(row)` создаёт
член (типорозмір), на member-документе `ChangeRow` переключает строку; вернуть выбранную строку и итоговый
размер (bbox/масса). Всё late-bound (reflection), защита copy-before-resize (как в `run_ilogic`).
Приёмка на реальном factory (напр. ВЦ/ВКР с таблицей) ещё нужна на 2026: уточнить точную сигнатуру
`CreateMember`/`ChangeRow` и колонку member на заводских таблицах.
